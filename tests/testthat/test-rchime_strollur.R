...
test_that("test rchime - errors and rchime_options", {
  ...
  expect_equal(strollur::count(data), 100)

  chimera_report <- strollur::xdev_report(data, type = "chimera_report")
  accnos_report <- strollur::xdev_report(data, type = "chimera_accnos")

  # checks to make sure the correct things are created
  expect_equal(nrow(accnos_report), 12)
  ...
  num_seqs <- strollur::count(data)
  rchime(data, reference = reference_ob)
  chimera_report <- strollur::xdev_report(data, type = "chimera_report")

  expect_equal((num_seqs - strollur::count(data)), 0)
  expect_equal(nrow(chimera_report), 3)
...
test_that("test rchime by reference - strollur", {
  ...
  expect_equal((num_seqs - strollur::count(data, distinct = TRUE)), 70)
...
test_that("test rchime de novo -strollur/mulitple samples, dereplicate TRUE", {
  ...
  expect_equal((num_seqs - strollur::count(data, distinct = TRUE)), 90)
  expect_equal(nrow(chimera_report), 500)
  expect_equal(
...
test_that("test rchime de novo strollur/mulitple samples, dereplicate FALSE", {
  ...
  num_seqs <- strollur::count(data, distinct = TRUE)
  rchime(data, verbose = FALSE, rchime_options = options)

  chimera_report <- strollur::xdev_report(data, type = "chimera_report")

  # checks to make sure the correct things are created
  expect_equal((num_seqs - strollur::count(data, distinct = TRUE)), 108)
  expect_equal(nrow(chimera_report), 500)
