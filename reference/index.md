# Package index

## Functions for detecting and removing chimeric sequences

Detect and remove chimeric sequences

- [`rchime()`](http://mothur.org/rchime/reference/rchime.md) : rchime

## Utility Functions

helper functions

- [`rchime_options()`](http://mothur.org/rchime/reference/rchime_options.md)
  :

  The
  [`rchime_options()`](http://mothur.org/rchime/reference/rchime_options.md)
  function allows you to set vsearch specific optional parameters.

- [`rchime_example()`](http://mothur.org/rchime/reference/rchime_example.md)
  : Get path to rchime example files

- [`silva_gold()`](http://mothur.org/rchime/reference/silva_gold.md) :
  silva_gold
